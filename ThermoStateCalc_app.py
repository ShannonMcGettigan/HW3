...
class main_window(QWidget,Ui__frm_StateCalculator):
    ...
    def SetupSlotsAndSignals(self):
        ...
        self._pb_Calculate_2.clicked.connect(self.calculateProperties)
    ...
    def setUnits(self):
        ...
        SP=[float(self._le_Property1_State1.text()), float(self._le_Property2_State1.text())]
        ...
        self._le_Property1_State1.setText("{:0.3f}".format(SP[0]))
        self._le_Property2_State1.setText("{:0.3f}".format(SP[1]))
        #endregion
    ...
    def calculateProperties(self):
        ...
        try:
            # -------- READ PROPERTIES --------
            SP = [
                self._cmb_Property1_State1.currentText().split("(")[-1][0].lower(),
                self._cmb_Property2_State1.currentText().split("(")[-1][0].lower()
            ]

            SP2 = [
                self._cmb_Property1_State2.currentText().split("(")[-1][0].lower(),
                self._cmb_Property2_State2.currentText().split("(")[-1][0].lower()
            ]

            # -------- READ VALUES --------
            f = [
                float(self._le_Property1_State1.text()),
                float(self._le_Property2_State1.text())
            ]

            f2 = [
                float(self._le_Property1_State2.text()),
                float(self._le_Property2_State2.text())
            ]
            ...
            # -------- VALIDATION --------
            if SP[0] == SP[1]:
                self._lbl_StateProperties.setText("Error: State 1 properties cannot be the same.")
                return

            if SP2[0] == SP2[1]:
                self._lbl_StateProperties.setText("Error: State 2 properties cannot be the same.")
                return
            ...
            # -------- OUTPUT --------
            text = "STATE 1:\n\n"
            text += self.makeLabel(self.state1)

            text += "\n\n----------------------\n\n"
            text += "STATE 2:\n\n"
            text += self.makeLabel(self.state2)

            text += "\n\n----------------------\n\n"
            text += "CHANGE:\n\n"
            text += self.makeDeltaLabel(self.state1, self.state2)

            self._lbl_StateProperties.setText(text)

        except Exception:
            self._lbl_StateProperties.setText("Error: Invalid state or inputs.")
#endregion
